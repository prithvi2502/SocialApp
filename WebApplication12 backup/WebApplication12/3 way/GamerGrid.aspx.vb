...
Public Class WebForm39
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        End If
        Dim cnn As SqlConnection = New SqlConnection("Integrated Security=SSPI;Data Source=BHUPPI-PC\SQLEXPRESS;Initial Catalog = games")
        Dim ids As String = email1.Text + "gamewin"
        Dim cmd As SqlDataAdapter = New SqlDataAdapter("SELECT [fname],[Lname],[email],[c1], [datemonth],[dateday],[dateyear],[thumbssup],[thumbsdown],[id],[shared],[comments],[favgame],[bornon],[hometown] FROM " + ids + "  order by [id] desc", cnn)
        Dim ds As DataSet = New DataSet()
        cmd.Fill(ds)
        repeater_code_Script.DataSource = ds
        repeater_code_Script.DataBind()
    ...
    Private Sub update_Click(sender As Object, e As System.EventArgs) Handles update.Click
        ...
        myconn = New SqlConnection("Integrated Security=SSPI;Data Source=BHUPPI-PC\SQLEXPRESS;Initial Catalog = games")
        Dim cmd As New SqlCommand("newcommentingame", myconn)
        ...
        cmd.Parameters.Add("@ccfavgame", SqlDbType.VarChar).Value = topic_name.Value.ToString()
    ...
    Protected Sub Repeater1_ItemCommand(source As Object, e As RepeaterCommandEventArgs)
        ...
        If e.CommandName = "Button1" Then
            Dim cnn2 As SqlConnection = New SqlConnection("Integrated Security=SSPI;Data Source=BHUPPI-PC\SQLEXPRESS;Initial Catalog = games")
            cnn2.Open()
            Dim cm1 As New SqlCommand
        ...
        End If
    End Sub
    Private Sub achievement_update_Click(sender As Object, e As System.EventArgs) Handles create_button.Click
        Dim myconn As New SqlConnection
        myconn = New SqlConnection("Integrated Security=SSPI;Data Source=BHUPPI-PC\SQLEXPRESS;Initial Catalog = games")
        Dim cmd As New SqlCommand("newcommentingame", myconn)
        cmd.CommandType = CommandType.StoredProcedure
        ...
        cmd.Parameters.Add("@cclname", SqlDbType.VarChar).Value = Label21.Text.ToString()
        cmd.Parameters.Add("@ccemail", SqlDbType.VarChar).Value = email1.Text
        cmd.Parameters.Add("@ccc1", SqlDbType.VarChar).Value = discription_var_input.Value.ToString()
        cmd.Parameters.Add("@ccdatemonth", SqlDbType.Int).Value = Date.Today.Month
        cmd.Parameters.Add("@ccdateday", SqlDbType.Int).Value = Date.Today.Day
        cmd.Parameters.Add("@ccdateyear", SqlDbType.Int).Value = Date.Today.Year
        cmd.Parameters.Add("@ccthumbsup", SqlDbType.Int).Value = "0"
        ...
        cmd.Parameters.Add("@cchometown", SqlDbType.VarChar).Value = file_type.Value
        ...
        Dim cnn As SqlConnection = New SqlConnection("Integrated Security=SSPI;Data Source=BHUPPI-PC\SQLEXPRESS;Initial Catalog = games")
        Dim ids As String = email1.Text + "gamewin"
        Dim cmd1 As SqlDataAdapter = New SqlDataAdapter("SELECT [fname],[Lname],[email],[c1], [datemonth],[dateday],[dateyear],[thumbssup],[thumbsdown],[id],[shared],[comments],[favgame],[bornon],[hometown] FROM " + ids + " where bornon = 2 order by [id] desc", cnn)
        Dim ds As DataSet = New DataSet()
        cmd1.Fill(ds)
        repeater_achievment_things.DataSource = ds
        repeater_achievment_things.DataBind()
        update_panel_achievement.Update()
    End Sub
End Class
